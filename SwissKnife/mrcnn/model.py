...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet50", "resnet101", "resnet150"]
    return np.array(
        [
            [
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    train_bn: Boolean. Train or freeze Batch Norm layers
    """
    assert architecture in ["resnet50", "resnet101", "resnet150"]
    # Stage 1
    ...
    block_count = {"resnet50": 5, "resnet101": 22, "resnet150": 50}[architecture]
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    asserts = [
        tf.Assert(
            tf.greater(tf.shape(proposals)[0], 0), [proposals], name="roi_assertion"
        ),
    ]
...
def load_image_gt(
    dataset, config, image_id, augment=False, augmentation=None, use_mini_mask=False
):
